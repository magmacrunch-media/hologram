...
constant int   FRESNEL_WINDOW = 3; /* HOLO_FRESNEL_WINDOW */
...
void fresnel_cylinder(float P0, float P1, float P2, float3 o, float3 d,
                      ...
                      thread float &best, thread float3 &best_n) {
    if (P2 < 1e-12) return;
...
bool ray_fresnel(float3 ro, float3 rd, float3 center, float3 axis,
    ...
    float best = 1e30;
    float3 best_n = float3(0.0, 0.0, 1.0);
...
/* The light a matte surface with normal n shows, per unit albedo: the flat
   ...
   cpu_trace.h. */
float3 matte_light(constant float4 *params, float3 n, float diffuse) {
    if (sky_light > 0.0) {
...
float3 trace(constant float4 *params, float3 ro, float3 rd) {
    ...
    while (sp > 0 && processed < MAX_RAYS) {
        ...
        if (matte > 0.0) {
            ...
            float3 lambert = albedo * matte_light(params, best_n, diffuse);
        ...
        while (processed < MAX_RAYS) {
            ...
            if (matte > 0.0) {
                ...
                float lambert = albedo_at(albedo, lambda_um)
                              * albedo_at(matte_light(params, best_n, diffuse),
                                          lambda_um);
